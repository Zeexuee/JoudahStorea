    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if($events->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-full transition hover:shadow-lg hover:-translate-y-1 duration-300">
                    ...
                    <div class="p-6 flex flex-col flex-grow">
                        <a href="{{ route('event.detail', $event->slug) }}" class="block mb-3">
                            <h3 class="text-xl font-bold text-gray-900 hover:text-amber-600 transition line-clamp-2" style="min-height: 3.5rem;">{{ $event->title }}</h3>
                        </a>
                        <p class="text-gray-600 text-sm mb-6 line-clamp-3" style="min-height: 3.75rem;">{{ Str::limit(strip_tags($event->description), 120) }}</p>
                        <div class="mt-auto flex items-center justify-between gap-3 pt-4 border-t border-gray-50">
                            <a href="{{ route('event.detail', $event->slug) }}" class="text-amber-600 hover:text-amber-700 font-medium text-sm transition shrink-0">
                                Baca Detail &rarr;
                            </a>
                            @if($event->external_url)
                                <a href="{{ $event->external_url }}" target="_blank" rel="noopener noreferrer" class="bg-gray-900 text-white hover:bg-gray-800 px-4 py-2 rounded text-xs font-semibold uppercase tracking-wider transition shrink-0">
                                    Go to Event
                                </a>
                            @endif
                        </div>
